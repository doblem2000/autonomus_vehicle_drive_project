    ...
    def run_step(self, input_data, timestamp):
        ...
        if not self._agent:
            ...
            if len(self.configs["SaveSpeedData"]) > 0:
                specific_simulation=os.environ.get('RESULT_PATH')
                if specific_simulation is not None:
                    file_path = specific_simulation + "/" + self.configs["SaveSpeedData"]
                else: 
                    file_path = "/workspace/team_code/" + self.configs["SaveSpeedData"]
